...
(function ($) {
    ...
    BootstrapTable.LOCALES['en-US'] = BootstrapTable.LOCALES.en = {
        ...
        formatLoadingMessage: function () {
            return '<i  class="fa fa-spinner fa-spin fa-2x fa-fw" ></i>加载数据中';
        },
    ...
    BootstrapTable.prototype.initContainer = function () {
        this.$container = $([
            ...
            '<div class="fixed-table-header"><table style="table-layout: fixed"></table></div>',
    ...
    BootstrapTable.prototype.remove2 = function (params) {
        ...
        for (i = len - 1; i >= 0; i--) {
            ...
            if ($.inArray(row[params.field], params.values) !== -1) {
                ...
                $("tr[data-uniqueid='"+row[params.field]+"']").remove();
            }
        ...
        $('input[name="btSelectItem"]').each(function(index,element){

            $(this).attr("data-index",index);
        })


        $('#bootstrap_table_body tr').each(function(index,element){
    ...
    BootstrapTable.prototype.updateRowData = function (params) {
        ...
        $.each(allParams, function(i, params) {
            ...
            var tempRow = that.getRowByUniqueId(params.id);
    ...
    //扩展编辑行
    BootstrapTable.prototype.updateRow2 = function (params) {
        var that = this;
        var allParams = $.isArray(params) ? params : [ params ];
        ...
        $.each(allParams, function(i, params) {
            ...
            var trHtml = that.getTrHtml(tempRow,rowId);



            $("tr[data-uniqueid='"+params.id+"']").replaceWith(trHtml);


        });

        var parentDom = $("#bootstrap_table_body");

        this.$selectItem = parentDom.find(sprintf('[name="%s"]', this.options.selectItemName));
    ...
    BootstrapTable.prototype.getTrHtml = function(item, i) {
        ...
        if (!this.options.cardView && this.options.detailView) {
            ...
            var temp = calculateObjectValue(null, this.options.detailFilter, [i, item]);
            ...
            if(temp===1){
                html.push('<i class="glyphicon glyphicon-file"></i>');
            }
        ...
        $.each(this.header.fields, function(j, field) {
            ...
            //alert("#########item=="+JSON.stringify(item)+"#########filed="+field);
            var text = '',
            ...
            if (column.checkbox || column.radio) {
                ...
                //div_td
                value = "<div class='div_td'>"+value+"</div>";
...
//扩展编辑行
    BootstrapTable.prototype.insertRow2 = function(item, i) {
        ...
        this.options.data = [].concat(item).concat(this.options.data);
        ...
        parentDom.prepend(html.join(' '));


        //this.updateSelected();
        //this.resetView();

        //this.trigger('post-body', this.data);
        ...
        //alert($('input[name="btSelectItem"]').length);
    ...
    BootstrapTable.prototype.initRow = function(item, i, data, parentDom) {
        ...
        $.each(this.header.fields, function(j, field) {
            ...
            if (item['_' + field + '_title']) {
                var tempTitle = item['_' + field + '_title'];
                tempTitle = (tempTitle == null ||tempTitle == "null" || tempTitle == "undefined"  || tempTitle == undefined || tempTitle=="")?"":tempTitle;
                title_ = sprintf(' title="%s"', tempTitle);
            ...
            if (column.checkbox || column.radio) {
                ...
                text = [sprintf(that.options.cardView ?
                    ...
                    sprintf(' disabled="%s"', !column.checkboxEnabled ||
                    (value && value.disabled) ? 'disabled' : undefined) +
                    //sprintf(' style="%s"', !column.checkboxEnabled ||
                    //(value && value.disabled) ? 'display:none' : undefined) +
                    ' />',
                    that.header.formatters[j] && typeof value === 'string' ? value : '',
                    that.options.cardView ? '</div>' : '</td>'
                ].join('');
                ...
                text = that.options.cardView ? ['<div class="card-view">',
                    ...
                    id_, class_, style, data_, rowspan_, colspan_, "title='"+value_+"'"),
                    value,
                    '</td>'
                ].join('');
    ...
    BootstrapTable.prototype.lockSort = function (isLock) {

        this.$lockSort = isLock;

    }

    BootstrapTable.prototype.initBody2 = function (params) {
    ...
    BootstrapTable.prototype.initBody = function (fixedScroll) {
        ...
        if (!this.$body.length) {
            this.$body = $('<tbody id="bootstrap_table_body" style="margin-bottom:100px;"></tbody>').appendTo(this.$el);
        }
    ...
    BootstrapTable.prototype.hideColumn2 = function (array) {
        //this.toggleColumn(this.fieldsColumnsIndex[field], false, true);
    ...
    BootstrapTable.prototype.showAllColumns2 = function (visible) {
        var that = this;
        $.each(this.columns, function (i, column) {
    ...
    BootstrapTable.prototype.refresh2 = function (params) {
    ...
    var allowedMethods = [
        ...
        'showColumn', 'hideColumn','hideColumn2', 'getHiddenColumns', 'getVisibleColumns',
        'showAllColumns', 'hideAllColumns',"showAllColumns2",
        'filterBy',
        'scrollTo',
        'getScrollPosition',
        ...
        'updateFormatText', 'updateCellById',"insertRow2","updateRow2","updateRowData","remove2","initBody","initBody2","lockSort","unlockSort"
    ];
