...
public class DynamicDataSourceAutoConfigure implements EnvironmentAware {
    ...
    // 分片配置文件名称
    @Value("${sharding.jdbc.filename:sharding-jdbc}")
    private String shardingFileName;
	...
	private void initShardingDataSource(Map<Object, Object> targetDataSources){
    	...
    	try {
    		...
    		Map<String, DataSource> dataSourceMap = new HashMap<>(); //设置分库映射
       	 	...
       	 	DataSource shardingDataSource = YamlShardingDataSourceFactory.createDataSource(dataSourceMap, ResourceUtils.getFile("classpath:"+shardingFileName+".yml"));
