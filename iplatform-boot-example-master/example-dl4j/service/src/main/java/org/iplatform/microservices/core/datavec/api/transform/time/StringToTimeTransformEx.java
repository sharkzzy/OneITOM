 ...
 *
 * 张磊修改，增加Locale locale参数
 *
 ******************************************************************************/

package org.iplatform.microservices.core.datavec.api.transform.time;
...
public class StringToTimeTransformEx extends BaseColumnTransform {
     ...
     */
    public StringToTimeTransformEx(String columnName, TimeZone timeZone, Locale locale) {
     ...
     * @param timeZone   Timezone for time parsing
     */
    public StringToTimeTransformEx(String columnName, String timeFormat, TimeZone timeZone, Locale locale) {
     ...
     * @param timeZone   Timezone for time parsing
     */
    public StringToTimeTransformEx(String columnName, DateTimeZone timeZone, Locale locale) {
        this(columnName, null, timeZone, locale, null, null);
    }
     ...
     * @param timeZone   Timezone for time parsing
     */
    public StringToTimeTransformEx(String columnName, String timeFormat, DateTimeZone timeZone, Locale locale) {
        this(columnName, timeFormat, timeZone, locale, null, null);
    }
     ...
     */
    public StringToTimeTransformEx(@JsonProperty("columnName") String columnName,
     ...
     * @param maxValidTime Max valid time (epoch millisecond format). If null: no restriction in max valid time
     */
    public StringToTimeTransformEx(String columnName, String timeFormat, DateTimeZone timeZone, Locale locale, Long minValidTime,
                                   Long maxValidTime) {
        super(columnName);
    ...
    public String toString() {
        ...
        sb.append("StringToTimeTransformEx(timeZone=").append(timeZone);
    ...
    private void readObject(ObjectInputStream in) throws IOException, ClassNotFoundException {
        ...
        if (timeFormat != null)
            if (this.locale != null) {
                this.formatter = DateTimeFormat.forPattern(timeFormat).withZone(timeZone).withLocale(this.locale);
            } else {
                this.formatter = DateTimeFormat.forPattern(timeFormat).withZone(timeZone);
            }
        else {
            ...
            for (int i = 0; i < formatters.length; i++) {
                if (this.locale != null) {
                    dateFormatList.add(DateTimeFormat.forPattern(formats[i]).withZone(timeZone).withLocale(this.locale));
                } else {
                    dateFormatList.add(DateTimeFormat.forPattern(formats[i]).withZone(timeZone));
                }
            }
