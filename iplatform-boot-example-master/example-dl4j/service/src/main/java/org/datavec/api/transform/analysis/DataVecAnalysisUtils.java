...
public class DataVecAnalysisUtils {
    ...
    public static List<ColumnAnalysis> convertCounters(List<AnalysisCounter> counters, double[][] minsMaxes, List<ColumnType> columnTypes){
        ...
        for (int i = 0; i < nColumns; i++) {
            ...
            switch (ct) {
                case String:
                    //org.datavec.spark.transform.analysis.string.StringAnalysisCounter sac = (org.datavec.spark.transform.analysis.string.StringAnalysisCounter) counters.get(i);
                    StringAnalysisCounter sac = (StringAnalysisCounter) counters.get(i); //以前出现过强转类型失败
